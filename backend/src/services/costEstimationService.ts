...
import { Destination, TripEstimate, TripEstimateBreakdown } from '../types';
...
const FLIGHT_DISTANCE_TIERS: Record<string, number> = {
...
const DESTINATION_DEMAND_FACTOR: Record<string, number> = {
  'new-york-ny': 50,
  'los-angeles-ca': 30,
  'miami-fl': 40,
  'las-vegas-nv': 30,
 ...
 */
export function calculateRoomsNeeded(travelers: number): number {
 ...
 */
export function estimateFlightPerPerson(
  destinationId: string,
  _origin: string // Currently unused, but available for future distance calculation
): number {
 ...
 */
export function estimateHotelPerNight(budgetScore: number): number {
...
export function calculateTripEstimate(
  ...
  const breakdown: TripEstimateBreakdown = {
 ...
 */
export function getTripEstimate(
  destination: Destination,
  origin: string,
  travelers: number,
  departureDate: string,
